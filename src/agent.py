...
async def my_agent(ctx: JobContext):
    ...
    async def hang_up_routine():
        ...
        try:
            ...
            # 1. Store final intent details into the Room metadata for Dialer tracking
            await lkapi.room.update_room_metadata(
                api.UpdateRoomMetadataRequest(
                    room=ctx.room.name,
                    metadata=call_state["status"]
                )
            )
            logger.info(f"Updated room metadata with status: {call_state['status']}")
            
            # 2. Safely disconnect the phone user to terminate trunk charges instantly
            phone_number = ctx.room.name.split("-")[1]
            sip_identity = f"sip-user-{phone_number}"
            try:
                await lkapi.room.remove_participant(
                    api.RoomParticipantIdentity(room=ctx.room.name, identity=sip_identity)
                )
            except Exception:
                pass
    ...
    fast_vad = silero.VAD.load(
        min_silence_duration=0.7,    # INCREASED: Wait 1 full second of silence before deciding the user stopped talking
        min_speech_duration=0.4,     # INCREASED: Ignore random short noises under 400ms (coughs, mic taps)
        prefix_padding_duration=0.1,    # ADDED: Default is usually 0.5. Higher = requires louder/clearer speech to trigger
    )
