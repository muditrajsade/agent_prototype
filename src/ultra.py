...
# --- New v1.5.x Imports for Ultravox ---
...
async def my_agent(ctx: JobContext):
    ...
    # 1. Initialize the AgentSession with Ultravox Realtime and Silero VAD
    session = AgentSession(
        vad=silero.VAD.load(), # Ultravox uses LiveKit's Silero plugin for turn detection
        llm=ultravox.realtime.RealtimeModel(
            model="fixie-ai/ultravox",
            voice="44504e63-59c5-4f69-9340-423231c79a03", # Change this to any other supported Ultravox voice if desired
            temperature=0.4 # Slightly lower temperature helps with more deterministic tool calling
        )
    )

    # 2. Define your agent's brain and attach tools
    my_instructions = """
